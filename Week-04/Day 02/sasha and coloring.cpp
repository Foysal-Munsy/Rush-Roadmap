...
ll spilt_and_sum(vl v)
{
    sort(v.begin() , v.end());
    ll mid = (v.size()/2);
    
    vl left (v.begin(), v.begin()+mid);
    vl right (v.begin() + mid + (v.size()%2), v.end());

    ll left_sum = accumulate(left.begin() , left.end() , 0LL);
    ll right_sum = accumulate(right.begin() , right.end() ,0LL);
